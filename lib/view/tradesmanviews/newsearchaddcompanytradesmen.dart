...
import 'package:bizbultest/services/Tradesmen/edit_tradesmen_controller.dart';
...
import '../../services/Tradesmen/new_add_company_controller.dart';

class NewSearchAddCompanyTradesmenLocationSearchPage
    extends GetView<AddTradesmenCompanyConttroller> {
  String countryName;
  String TextFiledData;
  NewSearchAddCompanyTradesmenLocationSearchPage(
      this.countryName, this.TextFiledData,
      {Key? key})
      : super(key: key);
  ...
  Widget _locationField() {
    return Container(
      ...
      child: TextFormField(
        onChanged: (val) async {
          controller.locationtext.value = val;

          await Future.delayed(
              Duration(
                milliseconds: 100,
              ), () async {
            controller.newlocations.value = await CountryAPI.fetchLocations(
  ...
  Widget _locationCard(int index, BuildContext context) {
    return ListTile(
      ...
      onTap: () {
        controller.locationtext.value = controller.newlocations[index].city;
        print("workarea=${controller.locationtext.value}");
        Navigator.of(context).pop({
          "city": controller.locationtext.value,
