...
class BannerAdWebView extends StatefulWidget {
  ...
  @override
  _BannerAdWebViewState createState() => _BannerAdWebViewState();
}

class _BannerAdWebViewState extends State<BannerAdWebView> {
  ...
  Future<void> confirmPayment(String token, String payerID) async {
    var url =
        Uri.parse("https://www.bebuzee.com/api/paypal_success_page_banner.php");
    ...
    var response = await http.post(url, body: {
      ...
      "action": "paypal_success_page_banner",
    });

    // final response = await http.post("https://www.bebuzee.com/app_develope_banner_ad.php", body: {
    //   "user_id": widget.memberID.toString(),
    //   "payer_id": payerID,
    //   "token": token,
    //   "ad_budget": widget.adBudget,
        ...
        return WebView(
          ...
          navigationDelegate: (NavigationRequest request) {
            if (request.url.startsWith(
              ...
              final endIndex1 = url.indexOf(end1, startIndex1 + start1.length);

              String token = url.substring(startIndex + start.length, endIndex);
              String payerID =
