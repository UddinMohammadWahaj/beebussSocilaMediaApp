...
class _ShortbuzCommentPageState extends State<ShortbuzCommentPage> {
  ...
  late ShortbuzModel feed;

  void getData() {
    if (widget.from == "Location") {
      setState(() {
        feed = new ShortbuzModel();
      });
  ...
  Future<void> getComments() async {
    ...
    if (response.statusCode == 200) {
      ...
      ShortbuzCommentModel commentsData =
          ShortbuzCommentModel.fromJson(jsonDecode(response.data));
  ...
  Future<ShortbuzCommentModel?> getSubComments(String commentID) async {
    var url =
        "https://www.bebuzee.com/api/newsfeed/postSubCommentData?action=post_comment_data&post_id=${feed.postId}&post_type=${feed.postType}&user_id=${feed.postUserId}";
    print(url);
    ...
    print(response.data);
    if (response.statusCode == 200) {
      ShortbuzCommentModel subCommentsData =
          ShortbuzCommentModel.fromJson(jsonDecode(response.data));
  ...
  Future<void> likeUnlikeSubcomment(String subcommentID) async {
    ...
    var client = new dio.Dio();
    print("token called");
    print("like unlike sub comment called");
    String? token =
        await ApiProvider().refreshToken(CurrentUser().currentUser.memberID!!);
    print("token: $token");
    var head = {
  ...
  Future<void> deleteComment(String commentID) async {
    var url =
        "https://www.bebuzee.com/api/newsfeed/postCommentDelete?action=delete_main_comment_data&user_id=${CurrentUser().currentUser.memberID!}&comment_id=$commentID";
  ...
  Future<void> postComment(String comment) async {
    var url =
        "https://www.bebuzee.com/api/newsfeed/postCommentAdd?action=post_comment_main&user_id=${CurrentUser().currentUser.memberID!}&post_type=${feed.postType}=&post_id=${feed.postId}&comments=$comment";
  ...
  Future<void> postReply(String commentID, String reply) async {
    var url =
        "api/newsfeed/postCommentReplay?action=post_comment_reply&post_type=${feed.postType}&post_id=${feed.postId}&user_id=${CurrentUser().currentUser.memberID!}&comment_id=$commentID&comments=$reply";
                            ...
                            child: TextFormField(
                              ...
                              decoration: InputDecoration(
                                ...
                                hintText: " " +
                                    ...
                                    '  ${CurrentUser().currentUser.shortcode.toString()}',
                                // 48 -> icon width
                              ),
                            ),
                          ),
