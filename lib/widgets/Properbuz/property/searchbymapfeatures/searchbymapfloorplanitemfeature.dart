...
class SearchByMapFloorPlanItemFeature extends GetView<SearchByMapController> {
  final int index;
  SearchByMapFloorPlanItemFeature({
    Key? key,
    required this.index,
  }) : super(key: key);
  ...
  Widget _imageCardCarousel(String image, int ind) {
    return GestureDetector(
      onTap: () {
        ...
        controller.propertylist[index]!.selectedFloorIndex!.value = ind;
      },
      child: Obx(
        () => Container(
          decoration: new BoxDecoration(
            ...
            border:
                ind == controller.propertylist[index]!.selectedFloorIndex!.value
                    ? new Border.all(
    ...
    return Positioned.fill(
      ...
      child: Align(
        ...
        child: Container(
          ...
          child: Obx(() => Text(
                "${(controller.propertylist[index]!.selectedFloorIndex!.value + 1).toString()}/${ctr.floorImagesNew.length}",
                style: TextStyle(
                    color: Colors.white,
                    fontSize: 14,
  ...
  Widget build(BuildContext context) {
    return ctr.floorImagesNew.isEmpty
        ? Center(
            child: Text("Floor Plan not Available"),
          )
        : Column(
