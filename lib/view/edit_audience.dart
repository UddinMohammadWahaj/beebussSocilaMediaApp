...
class EditAudience extends StatefulWidget {
  ...
  @override
  _EditAudienceState createState() => _EditAudienceState();
}

class _EditAudienceState extends State<EditAudience> {
  ...
  Future<String> getAudienceData() async {
    var newurl = Uri.parse(
        'https://www.bebuzee.com/api/audience_data_seperate.php?user_id=${widget.memberID}&audience_id=${widget.audienceId}');
    var client = Dio();
    String? token =
    ...
    var response = await client
        ...
        .then((value) => value);
    var url = Uri.parse(
        "https://www.bebuzee.com/app_devlope_all_boosted.php?action=get_audience_data_seperate&user_id=${widget.memberID}&audience_id=${widget.audienceId}");

    // var response = await http.get(url);

    if (response.statusCode == 200) {
      print("response of getaudience 2 = ${response.data}");
      setState(() {
        selectedMinAge = response.data['data']['age_from'].toInt();
        selectedMaxAge = response.data['data']['age_to'].toInt();
        selectedGender = response.data['data']['gender_type'];
        _nameController.text = response.data['data']['audience_name'];
        selectedLocations = response.data['data']['audience_location']
            .toString()
            .split('~~')
            .toList();
        hasAudience = true;
  ...
  Future<String> updateAudienceData() async {
    ...
    // var response = await http.get(url);
    print('saving response=${response.data}');
    if (response.statusCode == 200) {
      setState(() {
        print(response.data);
      });
  ...
  Future<List> getLocationData(String search) async {
    ...
    print('get edit location data ${response.data}');
    if (response.statusCode == 200) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: hasAudience == true
          ? SingleChildScrollView(
              child: Padding(
                padding:
                    EdgeInsets.symmetric(horizontal: 2.0.w, vertical: 4.0.h),
                child: Container(
                  child: Column(
