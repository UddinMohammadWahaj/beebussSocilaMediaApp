...
class BebuzeeShopMerchantInnerView extends StatefulWidget {
  var storeid;
  BebuzeeShopMerchantInnerView({Key? key, this.storeid}) : super(key: key);

  @override
  State<BebuzeeShopMerchantInnerView> createState() =>
      _BebuzeeShopMerchantInnerViewState();
}

class _BebuzeeShopMerchantInnerViewState
    extends State<BebuzeeShopMerchantInnerView> {
  @override
  Widget build(BuildContext context) {
    ...
    Widget subProductWidget(int index) {
      return ListTile(
        onTap: () {
          ...
          controller.currentFlagIcon.value =
              controller.countryList[index].flagIcon!;
          controller.currentCountryId.value =
              controller.countryList[index].countryValue!;
          print("store country id=${controller.currentCountryId}");
          Navigator.of(context).pop();
        },
        tileColor: Colors.white,
        leading:
            CircleAvatar(child: Text(controller.countryList[index].flagIcon!)),
        // shape:
        //     RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        title: Text('${controller.countryList[index].countryName} '),
      );
    }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        children: [
          ...
          Padding(
            ...
            child: TextField(
              controller: controller.currentStoreName,
          ...
          Obx(() => controller.pressedNext.value &&
                  controller.currentStoreName.value.text == ''
              ? Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: Text('Shop name is required',
          ...
          Card(
            child: ListTile(
              ...
              leading: CircleAvatar(
                backgroundColor: Colors.white,
                child: Obx(() => Text('${controller.currentFlagIcon}')),
              ),
              ...
              //   // ),
              // ),
              title: Obx(() => Text('${controller.currentCountry.value}')),
              trailing: IconButton(
                  onPressed: () {
                    controller.getCountry();
                    showBarModalBottomSheet(
                        ...
                        builder: (ctx) => Obx(
                              () => Container(
                                height: 50.0.h,
                                width: 100.0.w,
                                child: controller.countryList.length == 0
          ...
          Obx(() => controller.currentCountry.value == 'Select Country' &&
                  controller.pressedNext.value
              ? Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: Text('Store Country is required',
                      style: TextStyle(color: Colors.red)),
                )
              : Container())
        ],
      ...
      bottomNavigationBar: Obx(() => GestureDetector(
            onTap: () {
              controller.pressedNext.value = true;
              controller.storeid = widget.storeid;
              if (controller.currentStoreName.value.text != '')
                Navigator.of(context).push(MaterialPageRoute(
                    builder: (context) => BebuzeeMerchantInnerViewSecondStore(
