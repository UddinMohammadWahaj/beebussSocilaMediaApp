...
class NewAddTradesmenController extends GetxController {
  ...
  Future<List<ManageData>>? TradesManData;

  var tradesmenalbumlist = <TradesmenAlbum>[].obs;
  var tradesmenalbumnamecontroller = TextEditingController();
  ...
  void removeFromTradesmenAlbum(index) async {
    tradesmenalbumlist.removeAt(index);
    tradesmenalbumlist.refresh();
  }
  ...
  void addtoTradesmenAlbum() async {
    final byteCoverImage = albumCoverList[0].readAsBytesSync();
    print("album length=${albumCoverList.length}");
    var albumUrl = await TradesmanApi.getAlbumLink(albumCoverList[0]);
    tradesmenalbumlist.add(TradesmenAlbum(
        id: '',
        uintalbumPic: byteCoverImage,
        albumfilepic: albumCoverList[0],
        fileimages: <File>[].obs,
        images: <Uint8List>[].obs,
        albumpicUrl: albumUrl,
  ...
  covertToJson(TradesmenAlbum thiss) {
    var data = new Map<String, dynamic>();
    data['album_name'] = thiss.albumName;
    data['album_pic'] = 'testimg';
    base64Encode(thiss.uintalbumPic!);
    data['images'] = ['pelo'];
    thiss.images.map((element) => base64Encode(element!)).toList();
    print('after convert=${data}');
    return data;
  }

  void pickPhotosFiles({index: 0}) async {
    List<imgpicker.XFile> allFiles =
        await imgpicker.ImagePicker().pickMultiImage();
    List<File> imgFiles = [];
    imgFiles
        .addAll(tradesmenalbumlist[index].fileimages.value as Iterable<File>);
    for (int i = 0; i < allFiles.length; i++) {
      imgFiles.add(File(allFiles[i].path));
    }
    tradesmenalbumlist[index].fileimages.value = imgFiles;
    // photos.addAll(imgFiles);
    photos.value = imgFiles;
    tradesmenalbumlist[index].fileimages.refresh();

    print("=== ## photos $photos");
  }

  void pickPhotosFilesTest(index) async {
    List<imgpicker.XFile> allFiles =
        await imgpicker.ImagePicker().pickMultiImage();
    List<File> imgFiles = [];
    imgFiles.addAll(photos.value);
    for (int i = 0; i < allFiles.length; i++) {
      imgFiles.add(File(allFiles[i].path));
    }
...
class TradesmenAlbum {
  String? albumName;
  String? id;
  Uint8List? uintalbumPic;
  File? albumfilepic;
  String? albumpicUrl;
  String? albumpicsurllist;
  var images = <Uint8List?>[].obs;
  var fileimages = <File?>[].obs;

  TradesmenAlbum(
      {this.albumName,
      this.albumpicUrl,
      this.albumpicsurllist,
      this.id,
      this.albumfilepic,
      this.uintalbumPic,
  ...
  Map<String, dynamic> toJson() {
    ...
    data['album_pic'] = base64Encode(this.uintalbumPic!);
    data['images'] =
        this.images.map((element) => base64Encode(element!)).toList();
    return data;
  }
