...
import 'package:bizbultest/utilities/custom_icons_icons.dart';
...
class NewsFeedLikesPage extends StatefulWidget {
  final String? postID;
  ...
  @override
  _NewsFeedLikesPageState createState() => _NewsFeedLikesPageState();
}

class _NewsFeedLikesPageState extends State<NewsFeedLikesPage> {
  NewsFeedLikedUsers newsfeeduserList = NewsFeedLikedUsers([]);
  bool areUsersLoaded = false;
  late TextEditingController _searchController;
  RefreshController _refreshController =
      RefreshController(initialRefresh: false);
  late Future<NewsFeedLikedUsers> _userFuture;
  late NewsFeedLikedUsers _userList;

  void _newsfeedonLoading() async {
    NewsFeedLikedUsers? userData = await ProperbuzFeedsAPI.newsfeedonLoading(
        _userList, context, _refreshController, widget.postID!,
        postType: widget.postType!);
    if (userData != null) {
      setState(() {
        _userList.newsfeedusers.addAll(userData.newsfeedusers);
      });
    }
  }
  ...
  void initState() {
    ...
    _userFuture = ProperbuzFeedsAPI()
        .getUsersPropbuz("", widget.postID!, context, postType: widget.postType)
        .then((value) {
      setState(() {
        _userList = value;
      });
    ...
    _searchController = new TextEditingController()
      ..addListener(() {
        setState(() {
          _userFuture = ProperbuzFeedsAPI().getUsersPropbuz(
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  return SmartRefresher(
                    ...
                    child: _userList.newsfeedusers.length > 0
                        ? ListView.builder(
                            itemCount: _userList.newsfeedusers.length > 0
                                ? _userList.newsfeedusers.length + 1
                                : 0,
                            itemBuilder: (context, index) {
                              if (index == 0) {
                                ...
                                var user = _userList.newsfeedusers[index - 1];
                                return MemberCard(
                                  user: user,
                                  goToProfile: () {
                                    setState(() {
...
class MemberCard extends StatelessWidget {
  final NewsFeedLikedUsersModel? user;
