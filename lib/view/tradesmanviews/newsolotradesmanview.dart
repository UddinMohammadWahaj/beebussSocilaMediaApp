...
class NewSoloTradesmenView extends StatefulWidget {
  String companyType;
  String companyId;
  List preFillData;

  NewSoloTradesmenView(this.companyType, this.companyId, this.preFillData,
      {Key? key})
      : super(key: key);

  @override
  State<NewSoloTradesmenView> createState() => _NewSoloTradesmenViewState();
}

class _NewSoloTradesmenViewState extends State<NewSoloTradesmenView> {
  @override
  void initState() {
  ...
  Widget _nextButton(BuildContext context, StateSetter setsate) {
    return GestureDetector(
      onTap: () async {
        // print(
        //     "clicking on done albumlist,${ctr.covertToJson(ctr.tradesmenalbumlist[0])}");
        // var data = ctr.tradesmenalbumlist
        //     .map((element) => ctr.covertToJson(element))
        //     .toList();
        // print("full data=${data}");
        List<String> dataalbum = [];
        print("\"hello\"");

        for (int i = 0; i < ctr.tradesmenalbumlist.length; i++) {
          dataalbum.add(jsonEncode({
            "album_name": ctr.tradesmenalbumlist[i].albumName,
            "album_pic": ctr.tradesmenalbumlist[i].albumpicUrl,
            "images": ctr.tradesmenalbumlist[i].albumpicsurllist ?? ''
          }));
        }
        var albummedia = dataalbum;
        print('albummedia=${albummedia}');
        ...
        var totdatat = {
          ...
          "album_media": albummedia.toString(),
        ...
        // logDev.log(
        //     "album_pal=${jsonEncode(ctr.tradesmenalbumlist.map((element) => ctr.covertToJson(element)).toList())}",
        //     name: "myLog");
        // printWrapped(
        //     "album_media=${jsonEncode(ctr.tradesmenalbumlist.map((element) => ctr.covertToJson(element)).toList())}");
        // return;
        // var halfdata2 = {
        //   "full_name": nameController.text,
        //   "user_id": CurrentUser().currentUser.memberID,
        ...
        //   "insurance": ctr.workUndertaking.value ? 'Yes' : 'No',
        //   "undertaken": ctr.workUndertaking.value ? 'Yes' : 'No',
        //   "album_media": jsonEncode(ctr.tradesmenalbumlist
        //       .map((element) => ctr.covertToJson(element))
        //       .toList()),
        //   "work_area": jsonEncode(workArea),
        //   // "work_subcategory": workSubCatController.text,
        //   "work_subcategory": jsonEncode(subCat)
        // };
        // print("halfdata=${haldata}");
          ...
          child: Container(
              ...
              child: Column(
                children: [
                            ...
                            builder: (context) => Container(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      _customTextFieldNew(
                                          AppLocalizations.of("Album name"),
                                          ctr.tradesmenalbumnamecontroller),
                                      Text(
                                      ...
                                      Card(
                                        child: Obx(
                                          () => ListTile(
                                            leading: ctr.albumCoverList.value
                                                        .length !=
                                                    0
                                                ? CircleAvatar(
                                                    backgroundImage: FileImage(
                                                    ctr.albumCoverList[0],
                                                  ))
                                                //  Container(
                                                //     height: 12.0.h,
                                                //     width: 17.0.w,
                  ...
                  Obx(() => ctr.tradesmenalbumlist.length > 0
                      ? ListView.builder(
                          shrinkWrap: true,
                          physics: NeverScrollableScrollPhysics(),
                          itemCount: ctr.tradesmenalbumlist.length,
                          itemBuilder: (ctx, index) => Card(
                            child: ListTile(
                              leading: Container(
                                  ...
                                  width: 10.0.w,
                                  child: Image.memory(
                                    ctr.tradesmenalbumlist[index].uintalbumPic!,
                                    fit: BoxFit.cover,
                                  )),
                              title: Text(
                                  ctr.tradesmenalbumlist[index].albumName!),
                              trailing: InkWell(
                                  onTap: () {
                                    showBarModalBottomSheet(
                                        context: context,
                                                                ...
                                                                () => ListTile(
                                                                  leading: ctr
                                                                      ...
                                                                      ? TextButton(
                                                                          onPressed:
                                                                            ...
                                                                            // ctr.photos.forEach((element) {
                                                                            //   dummydata.add(element);
                                                                            // });
                                                                            // ctr.tradesmenalbumlist[index].fileimages.value =
                                                                            //     dummydata;
                                                                            // dummydata.forEach((element) {
                                                                            //   byteimagelist.add(element.readAsBytesSync());
                                                                            // });
                                                                            // ctr.tradesmenalbumlist[index].images.value =
                                                                            //     byteimagelist;
                                                                            Navigator.of(context).pop();
                                                                            var urllist =
                                                                                await TradesmanApi.getMultipleImagesLink(ctr.tradesmenalbumlist[index].fileimages as List<File>);
                                                                            print("urllist=${urllist}");
                                                                            ctr.tradesmenalbumlist[index].albumpicsurllist =
                                                                                urllist;
                                                                            ctr.tradesmenalbumlist[index].fileimages.refresh();

                                                                            ctr.tradesmenalbumlist[index].fileimages.forEach((e) {
                                                                              byteimagelist.add(e!.readAsBytesSync());
                                                                            });
                                                                            ctr.tradesmenalbumlist[index].images.value =
                                                                                byteimagelist;
                                                                            print("added to current inndex=$index");
                                                                          },
                                                                          child: Text(
                                                                              'DONE'))
                                                                      : SizedBox(
                                                                  ...
                                                                  trailing:
                                                                      IconButton(
                                                                          onPressed:
                                                                              () {
                                                                            // ctr.tradesmenalbumlist.refresh();
                                                                            Navigator.of(context).pop();
                                                                          },
                                                                          icon:
                                                                              Icon(Icons.close)),
                                                              ...
                                                              Obx(
                                                                () => Padding(
                                                                  ...
                                                                  child:
                                                                    ...
                                                                    width:
                                                                        100.0.w,
                                                                    child: ctr.tradesmenalbumlist[index].fileimages.length ==
                                                                            0
                                                                        ? Container()
                                                                        : GridView.builder(
                                                                            shrinkWrap: true,
                                                                            ...
                                                                            itemCount: ctr.tradesmenalbumlist[index].fileimages.length,
                                                                            itemBuilder: (ctx, ind) => Container(
                                                                                  ...
                                                                                  child: Stack(
                                                                                    children: [
                                                                                      Container(
                                                                                        decoration: BoxDecoration(image: DecorationImage(fit: BoxFit.cover, image: FileImage(ctr.tradesmenalbumlist[index].fileimages[ind]!))),
                                                                                        height: 20.0.h,
                                                                                        width: 25.0.w,
                                                                                      ),
                                                                                      InkWell(
                                                                                        onTap: () {
                                                                                          ctr.tradesmenalbumlist[index].fileimages.removeAt(ind);
                                                                                          ctr.tradesmenalbumlist[index].fileimages.refresh();
                                                                                        },
                                                                                        child: CircleAvatar(
