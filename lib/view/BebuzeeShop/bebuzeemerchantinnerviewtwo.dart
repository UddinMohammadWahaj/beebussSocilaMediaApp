...
class BebuzeeMerchantInnerViewSecond extends StatefulWidget {
  BebuzeeShopMerchantController? controller;
  BebuzeeMerchantInnerViewSecond({Key? key, this.controller}) : super(key: key);

  @override
  State<BebuzeeMerchantInnerViewSecond> createState() =>
      _BebuzeeMerchantInnerViewSecondState();
}

class _BebuzeeMerchantInnerViewSecondState
    extends State<BebuzeeMerchantInnerViewSecond> {
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
                                  ...
                                  decoration: BoxDecoration(
                                      image: DecorationImage(
                                          ...
                                          image: FileImage(File(
                                              '${widget.controller!.photos[index]!.path}')))),
                                  height: 30.0.h,
                                  width: 90.0.w,
                                ),
                                IconButton(
                                    onPressed: () {
                                      widget.controller!.photos.removeAt(index);
                                      widget.controller!.photos.refresh();
                                    },
                                    icon: Icon(
                                      Icons.delete,
            ...
            Obx(() => widget.controller!.photos.length == 0 &&
                    widget.controller!.pressedAddProduct.value
                ? Padding(
                    padding: const EdgeInsets.all(8.0),
                    child: Text('Product images are required',
            ...
            //   ),
            // ),
            // Obx(() => widget.controller.currentCurrency.value == '' &&
            //         widget.controller.pressedAddProduct.value
            //     ? Padding(
            //         padding: const EdgeInsets.all(8.0),
            //         child: Text('Product currency is required',
            //             style: TextStyle(color: Colors.red)),
            //       )
            //     : Container()),
            Padding(
            ...
            Obx(() => widget.controller!.productPrice.text == '' &&
                    widget.controller!.pressedAddProduct.value
                ? Padding(
                    padding: const EdgeInsets.all(8.0),
                    child: Text('Product price is required',
                        style: TextStyle(color: Colors.red)),
                  )
                : Container()),
            Padding(
            ...
            Obx(() => widget.controller!.productSellingPrice.text == '' &&
                    widget.controller!.pressedAddProduct.value
                ? Padding(
                    padding: const EdgeInsets.all(8.0),
                    child: Text('Product selling price is required',
                        style: TextStyle(color: Colors.red)),
                  )
                : Container()),
            Padding(
            ...
            Card(
              child: ListTile(
                ...
                title: Obx(() =>
                    Text('${widget.controller!.currentProductCategory.value}')),
                trailing: IconButton(
                    onPressed: () {
                                        ...
                                        itemBuilder: (context, index) =>
                                            ListTile(
                                              ...
                                              leading: CircleAvatar(
                                                backgroundImage:
                                                    CachedNetworkImageProvider(
                                                  '${widget.controller!.productCategorylist[index].categoryImage}',
                                                ),
                                              ),
                                              title: Text(
            ...
            Obx(() => widget.controller!.currentCategory.value != '' &&
                ...
                ? Padding(
                    padding: const EdgeInsets.all(8.0),
                    child: Text('Product category is required',
                        style: TextStyle(color: Colors.red)),
                  )
                : Container()),
            Padding(
            ...
            Obx(() =>
                widget.controller!.currentProductSubCategory.value != '' &&
                    ...
                    ? Padding(
                        padding: const EdgeInsets.all(8.0),
                        child: Text('Product sub category is required',
                            style: TextStyle(color: Colors.red)),
                      )
                    : Container()),
            Padding(
              padding: const EdgeInsets.all(8.0),
              child: Text('Add Product Title',
                  style: TextStyle(
                      fontWeight: FontWeight.w500, color: Colors.grey)),
            ),
            ...
            Obx(() => widget.controller!.pressedAddProduct.value &&
                    widget.controller!.productname.text != ''
                ? Padding(
                    padding: const EdgeInsets.all(8.0),
                    child: Text('Product title is required',
                        style: TextStyle(color: Colors.red)),
                  )
                : Container()),
            Padding(
            ...
            Obx(() => widget.controller!.pressedAddProduct.value &&
                    widget.controller!.currentStoreUrl.text == ''
                ? Padding(
                    padding: const EdgeInsets.all(8.0),
                    child: Text('Product url is required',
                        style: TextStyle(color: Colors.red)),
                  )
                : Container()),
            // Obx(() => widget.controller.currentStoreUrl.text != '' &&
            //         widget.controller.pressedAddProduct.value
            //     ? Padding(
            //         padding: const EdgeInsets.all(8.0),
            //         child: Text('Product url is required',
            ...
            Obx(
              () => widget.controller!.pressedAddProduct.value &&
                      widget.controller!.productBrand.text == ''
                  ? Padding(
                      padding: const EdgeInsets.all(8.0),
                      child: Text('Product brand is required',
                          style: TextStyle(color: Colors.red)),
                    )
                  : Container(),
            ),
            ...
            Padding(
              padding: const EdgeInsets.all(8.0),
              child: Text('Product description is required',
                  style: TextStyle(color: Colors.red)),
            ),
          ],
        ),
      ...
      bottomNavigationBar: GestureDetector(
        onTap: () {
          // widget.controller.getSelectedColors();
          // return;

          widget.controller!.pressedAddProduct.value = true;
          if (widget.controller!.photos.length != 0 &&
              // widget.controller.currentCurrency.value != '' &&
              widget.controller!.productPrice.text != '' &&
              widget.controller!.currentProductCategory.value != '' &&
              widget.controller!.currentProductSubCategory.value != '' &&
              widget.controller!.currentStoreUrl.text != '' &&
              widget.controller!.productBrand.text != '' &&
              widget.controller!.productDescription.value != '') {
            Navigator.of(context).push(MaterialPageRoute(
            ...
            widget.controller!.refreshAddProduct();
          }
        },
        child: ClipRRect(
