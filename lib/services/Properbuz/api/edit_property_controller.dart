...
class EditPropertyController extends GetxController {
  final propId;
  EditPropertyController(this.propId);
  var photos = [].obs;
  var floorplanphotos = [].obs;
  var pickedphotos = [].obs;
  ...
  var isproperyPaid = {"name": "Property paid", "value": false.obs}.obs;
  ...
  var issharedOwnership = {"name": "Shared Ownership", "value": false.obs}.obs;

  var isretirementHome = {"name": "Retirement Homes", "value": false.obs}.obs;

  var underOfferSold = {"name": "Under Offer or sold", "value": false.obs}.obs;
  ...
  //   print("---- response = ${message.value} -- ${currentCountry}");
  //   if (message.value.isNotEmpty) {
  //     print("---- response = ${message.value} -- ${currentCountry}");
  //     isLocationLoading.value = true;
  //     await AddPropertyAPI.fetchLocations(message.value, currentCountry.value)
  //         .then((value) {
  ...
  List<File> getImageFiles(list) {
    List<File> imgFileList = [];
    print("----imf -- $photos");
    list.forEach((element) {
      if (element.runtimeType.toString() != "String") {
        print("----imgg -- ${element.runtimeType}");
        imgFileList.add(element);
      }
    });
    if (imgFileList.length == 0) return [];
    return imgFileList;
  }

  void updateData(
      propId, Map<String, dynamic> dataMap, Function callback) async {
    isaddLoad.value = true;
    List<File> imgFileList = getImageFiles(photos);
    print('list file image $imgFileList');

    var resp = await AddPropertyAPI.updateProperty(
      propId,
      dataMap,
      imgFileList,
      imgPicked.value,
...
//
  ...
  // void postData(Map<String, dynamic> dataMap, Function callback) async {
  //   print("here postdata called${photos.length}");

  //   var resp = await AddPropertyAPI.postAddProperty(dataMap, photos.value)
  //       .then((value) => value);
  ...
  var imgPicked = false.obs;
  var floorPicked = false.obs;

  void pickPhotosFiles() async {
    print("called pick photo--- $photos");
    // photos.value = [];
    ...
    for (int i = 0; i < allFiles.length; i++) {
      ...
      imgPicked.value = true;
      print("length=${imgFiles.length}");
  ...
  void pickPhotosFilesFloor() async {
    print("called pick photo");
    List<XFile> allFiles = await ImagePicker().pickMultiImage();
    List<File> imgFiles = [];
    List pickedFile = [];
    for (int i = 0; i < allFiles.length; i++) {
      imgFiles.add(File(allFiles[i].path));
      floorPicked.value = true;
      print("length=${imgFiles.length}");
      //     (allowMultiple: true, type: FileType.image);
      // List<File> allFiles = result.paths.map((path) => File(path)).toList();
      // photos.value = allFiles;
    ...
    if (floorplanphotos.length == 0)
      ...
      floorplanphotos.forEach((element) {
        pickedFile.add(element);
      });
      imgFiles.forEach((element) {
        pickedFile.add(element);
      ...
      floorplanphotos.value = pickedFile;
      if (floorplanphotos == null) {
        print("null photo");
      } else {
        print(
            "image added ${floorplanphotos.first} length ${floorplanphotos.length}");
      }
    }
  }
  ...
  void deleteFile(int index, list) async {
    selectedPge.value = index - 1;
    await AddPropertyAPI.deleteImage(list[index], this.propId);
    photos.removeAt(index);
  }

  void deleteFloorFile(int index) async {
    selectedPge.value = index - 1;
    await AddPropertyAPI.deleteImage(floorplanphotos[index], this.propId);
    photos.removeAt(index);
  }
...
//LIST 1
      print("currenfeature ID is not empty");

      fixtureList1.forEach((element) {
        bool match = false;
        for (int i = 0; i < currentfixtureID.length; i++) {
          if (element.fixtureID == currentfixtureID[i]) {
            fixture1iconlist.add(Icon(
    ...
    // for (int i = 0, j = 0; i < fixtureList1.length; i++) {
    //   if (j < currentfixtureID.length &&
    //       currentfixtureID[j] == fixtureList1.length) {
    //     fixture1iconlist.add(Icon(
    //       CupertinoIcons.square_fill,
    //       color: settingsColor,
    //     ));
    ...
    //     ));
    // }
    // for (int i = 0, j = 0; i < fixtureList2.length; i++) {
    //   if (j < currentfixtureID.length &&
    //       currentfixtureID[j] == fixtureList2[i].fixtureID) {
    //     fixture2iconlist.add(Icon(
    //       CupertinoIcons.square_fill,
    //       color: settingsColor,
    //     ));
  ...
  void fetchOutinspaceList() async {
    ...
    if (currentoutinspaceID.length == 0) {
...
//LIST 1
      ...
      outinspaceList1.forEach((element) {
        bool match = false;
        for (int i = 0; i < currentoutinspaceID.length; i++) {
          if (element.outinspaceID == currentoutinspaceID[i]) {
            outinspace1iconlist.add(Icon(
              CupertinoIcons.square_fill,
              color: hotPropertiesThemeColor,
            ));
      ...
      print("otinspace2.length=${outinspaceList2.length}");
      outinspaceList2.forEach((element) {
        bool match = false;
        for (int i = 0; i < currentoutinspaceID.length; i++) {
          if (element.outinspaceID == currentoutinspaceID[i]) {
            outinspace2iconlist.add(Icon(
      ...
      outinspace2iconlist.forEach((element) {
        print("icon la ${element.icon}");
      });
      print("exited fucntion");
    }

    // for (int i = 0, j = 0; i < outinspaceList1.length; i++) {
    //   if (j < currentoutinspaceID.length &&
    //       currentoutinspaceID[j] == outinspaceList1[i].outinspaceID) {
    //     outinspace1iconlist.add(Icon(
    //       CupertinoIcons.square_fill,
    //       color: settingsColor,
    //     ));
    ...
    // for (int i = 0, j = 0; i < outinspaceList2.length; i++) {
    //   if (j < currentoutinspaceID.length &&
    //       currentoutinspaceID[j] == outinspaceList2[i]) {
    //     outinspace2iconlist.add(Icon(
    //       CupertinoIcons.square_fill,
    //       color: settingsColor,
    //     ));
  ...
  void fetchFacilities() async {
    ...
    if (currentfacilitiesID.length == 0) {
...
//LIST 1
      ...
      facilitiesList1.forEach((element) {
        bool match = false;
        for (int i = 0; i < currentfacilitiesID.length; i++) {
          if (element.facilityID == currentfacilitiesID[i]) {
            facilities1iconlist.add(Icon(
              CupertinoIcons.square_fill,
              color: hotPropertiesThemeColor,
            ));
      ...
      facilitiesList2.forEach((element) {
        bool match = false;
        for (int i = 0; i < currentfacilitiesID.length; i++) {
          if (element.facilityID == currentfacilitiesID[i]) {
            facilities2iconlist.add(Icon(
    ...
    // for (int i = 0, j = 0; i < facilitiesList1.length; i++) {
    //   if (j < currentfacilitiesID.length &&
    //       currentfacilitiesID[i] == facilitiesList1[i].facilityID) {
    //     facilities1iconlist.add(Icon(
    //       CupertinoIcons.square_fill,
    //       color: settingsColor,
    //     ));
    ...
    //     ));
    // }
    // for (int i = 0, j = 0; i < facilitiesList2.length; i++) {
    //   if (j < currentfacilitiesID.length &&
    //       facilitiesList2[i].facilityID == currentfacilitiesID[j]) {
    //     facilities2iconlist.add(Icon(
    //       CupertinoIcons.square_fill,
    //       color: settingsColor,
    //     ));
  ...
  void getSelectedFacilities() {
    ...
    for (int i = 0; i < facilities2iconlist.length; i++) {
  ...
  void checkMatch(List l1, List l2) {}

  var currentfeatureID = [].obs;
  var currentfixtureID = [].obs;
  var currentoutinspaceID = [].obs;
  var currentfacilitiesID = [].obs;
  void fetchSpecialFeatures() async {
    ...
    // datalist = datalist.where((element) => element.featureID == featureID);
...
//LIST 1
      ...
      featureList1.forEach((element) {
        bool match = false;
        for (int i = 0; i < currentfeatureID.length; i++) {
          if (element.featureID == currentfeatureID[i]) {
            feature1iconlist.add(Icon(
              CupertinoIcons.square_fill,
              color: hotPropertiesThemeColor,
            ));
  ...
  String parseDate(DateTime propdate) {
    var month = propdate.month;
    var day = propdate.day;
    var year = propdate.year;
  ...
  var currentyCountryId = "".obs;
  var editLoding = false.obs;

  void fetchEditPropertyDetail(propId) async {
    editLoding.value = true;
    var data = await AddPropertyAPI.getEditPropertyDetail(propId);
    // print("------ 331 ${data.countryId}");

    editLoding.value = false;

    currentyCountryId.value = data.countryId ?? "";
    currentBathroom.value = data.bathrooms ?? "";
    currentBedroom.value = data.bedrooms ?? "";
    currentCountry.value = data.country ?? "";
    currentCountryIndex.value = int.parse(data.countryId);
    currentCurrency.value = data.currency ?? "";
    currentFloor.value = data.floor ?? "";
    currentListingType.value = data.typeId == "1" ? "Sale" : "Rental";
    currentListingTypeIndex.value = data.typeId == "1" ? 0 : 1;
    currentPropertyType.value = data.propertyType ?? "";
    // = data.propertyTypeId;

    currentfeatureID.value = parseCSV(data.specialFeatures).length > 0
        ? parseCSV(data.specialFeatures)
        : [];
    currentoutinspaceID.value = parseCSV(data.outdoorIndoorSpace).length > 0
        ? parseCSV(data.outdoorIndoorSpace)
        : [];
    currentfixtureID.value = parseCSV(data.fixtureFitting).length > 0
        ? parseCSV(data.fixtureFitting)
        : [];
    currentfacilitiesID.value =
        parseCSV(data.facility).length > 0 ? parseCSV(data.facility) : [];
    currentDimension.value = data.landDimensionMeasure ?? "";
    currentPreference.value = data.preference ?? "";
    isListedProp['value'].value = data.homeListed == "Yes" ? true : false;
    isFurnished.value = data.furnished == "Yes" ? true : false;
    propertyDescription.text = data.propertyDescription ?? "";
    propertyTitle.text = data.propertyTitle ?? "";
    propertyZipCode.text = data.postcode ?? "";
    propertyLat.text = data.latitude ?? "";
    propertyLong.text = data.longitude ?? "";
    propertyLandArea.text = data.landAreaMeasure ?? "";
    currentLandArea.value = data.landAreaMeasure ?? "";
    propertyStreet2.text = data.streetName2 ?? "";
    message.value = data.location ?? "";
    propertStreet1.text = data.streetName1 ?? "";
    propertyNeighbourhood.text = data.propertyNeighboorhood ?? "";
    propertyHouseName.text = data.propertyTitle ?? "";
    propertyWidth.text = data.builtUpDimensionWidth ?? "";
    propertyLength.text = data.builtUpDimensionHeight ?? "";
    propertyPrice.text = data.cost ?? "";
    photos.value = data.images ?? "";
    floorplanphotos.value = data.floorimages ?? "";
    selectedDate.value = (data.propertyAvailableDate != null)
        ? parseDate(data.propertyAvailableDate)
        : "";
    print("is property paid=${data.proPaid} && lala=${isproperyPaid['value']}");
    isproperyPaid.value = {
      "name": "Property Paid",
      "value": data.proPaid == "1" ? true.obs : true.obs
    };
    issharedOwnership.value = {
      "name": "Shared Ownership",
      "value": data.sharedOwnership == "Yes" ? true.obs : false.obs
    };
    isretirementHome.value = {
      "name": "Retirement Home",
      "value": data.retHomes == "Yes" ? true.obs : false.obs
    };
    underOfferSold.value = {
      "name": "Under Offer or sold",
      "value": data.undOffer == "Yes" ? true.obs : false.obs
    };

    // issharedOwnership['value'] =
    //     data.sharedOwnership == "Yes" ? true.obs : false.obs;
    print("------ 33 ");
  ...
  void onInit() {
    ...
    // fetchEditPropertyDetail(propId);
