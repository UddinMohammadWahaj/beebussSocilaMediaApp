...
class SearchByMapPhotosItemFeature extends GetView<SearchByMapController> {
  final int val;
  final int index;

  const SearchByMapPhotosItemFeature({
    Key? key,
    required this.val,
    required this.index,
  }) : super(key: key);
  ...
  Widget _imageCardCarousel(String image, int ind) {
    return GestureDetector(
      onTap: () {
        ...
        controller.propertylist[index]!.selectedPhotoIndex!.value = ind;
      },
      child: Obx(
        () => Container(
          decoration: new BoxDecoration(
            ...
            border:
                ind == controller.propertylist[index]!.selectedPhotoIndex!.value
                    ? new Border.all(
    ...
    return Positioned.fill(
      ...
      child: Align(
        ...
        child: Container(
          ...
          child: Obx(() => Text(
                "${(controller.propertylist[index]!.selectedPhotoIndex!.value + 1).toString()}/${controller.propertylist[index]!.images!.length}",
                style: TextStyle(
                    color: Colors.white,
                    fontSize: 14,
                ...
                () => PageView.builder(
                    ...
                    itemBuilder: (context, imageIndex) {
                      return _imageCardPageView(
                          controller.propertylist[index]!.images![imageIndex]);
                    }),
              ),
            ),
            _pageCard(),
          ...
          () => CarouselSlider.builder(
              ...
              itemBuilder: (context, value, ind) {
                return _imageCardCarousel(
                    controller.propertylist[index]!.images![value], value);
              },
